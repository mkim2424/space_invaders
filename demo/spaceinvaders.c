...
void on_key(char key, KeyEventType type, double held_time, void *aux) {
    ...
    if (type == KEY_PRESSED) {
        switch (key) {
            ...
            case ' ':
                projectile = rectangle_shape((Vector) {.x = body_get_centroid(player).x, .y = 2 * PLAYER_HEIGHT},
...
void check_projectile_boundary(Body *projectile) {
...
void check_boundary(Scene *scene) {
    ...
    for (int i = 0; i < scene_bodies(scene); i++) {
        ...
        else if ((* (Body_info *) body_get_info(scene_get_body(scene, i))).b == PROJECTILE) {
            projectile = scene_get_body(scene, i);
            check_projectile_boundary(projectile);
        }
    }
}
    ...
    for (int i = 0; i < scene_bodies(scene); i++) {
        if ((* (Body_info *) body_get_info(scene_get_body(scene, i))).b == ENEMY) {
            if (temp == random_enemy) {
                Body *projectile = rectangle_shape(body_get_centroid(scene_get_body(scene, i)),
...
int main(int argc, char *argv[]) {
    ...
    // Make player
    Body *player = rectangle_shape((Vector) {.x = WIDTH/2, .y = PLAYER_HEIGHT},
