...
/**
 * Returns the vector located at ptr.
 *
 * @param ptr a void * pointer that points to a vector on the heap.
 */
Vector void_to_vec(void *ptr) {
    return *((Vector *) ptr);
}
...
List *make_square() {
    ...
    Vector *vec1 = vec_init(1, 1);
    Vector *vec2 = vec_init(-1, 1);
    Vector *vec3 = vec_init(-1, -1);
    Vector *vec4 = vec_init(1, -1);
    list_add(sq, vec1);
    list_add(sq, vec2);
    list_add(sq, vec3);
    list_add(sq, vec4);
...
void test_square_translate() {
    ...
    assert(vec_equal(void_to_vec(list_get(sq, 0)), (Vector){3, 4}));
    assert(vec_equal(void_to_vec(list_get(sq, 1)), (Vector){1, 4}));
    assert(vec_equal(void_to_vec(list_get(sq, 2)), (Vector){1, 2}));
    assert(vec_equal(void_to_vec(list_get(sq, 3)), (Vector){3, 2}));
    assert(isclose(polygon_area(sq), 4));
    assert(vec_isclose(polygon_centroid(sq), (Vector){2, 3}));
    list_free(sq);
}
...
void test_square_rotate() {
    ...
    assert(vec_isclose(void_to_vec(list_get(sq, 0)), (Vector){0, sqrt(2)}));
    assert(vec_isclose(void_to_vec(list_get(sq, 1)), (Vector){-sqrt(2), 0}));
    assert(vec_isclose(void_to_vec(list_get(sq, 2)), (Vector){0, -sqrt(2)}));
    assert(vec_isclose(void_to_vec(list_get(sq, 3)), (Vector){sqrt(2), 0}));
    assert(isclose(polygon_area(sq), 4));
    assert(vec_isclose(polygon_centroid(sq), VEC_ZERO));
    list_free(sq);
}
...
// Make 3-4-5 triangle
List *make_triangle() {
    List *tri = list_init(3, (FreeFunc) vec_free);
    Vector *vec1 = vec_init(0, 0);
    Vector *vec2 = vec_init(4, 0);
    Vector *vec3 = vec_init(4, 3);
    list_add(tri, vec1);
    list_add(tri, vec2);
    list_add(tri, vec3);
    return tri;
...
void test_triangle_translate() {
    ...
    assert(vec_equal(void_to_vec(list_get(tri, 0)), (Vector){-4, -3}));
    assert(vec_equal(void_to_vec(list_get(tri, 1)), (Vector){0,  -3}));
    assert(vec_equal(void_to_vec(list_get(tri, 2)), (Vector){0,  0}));
    assert(isclose(polygon_area(tri), 6));
...
void test_triangle_rotate() {
    ...
    assert(vec_isclose(void_to_vec(list_get(tri, 0)), (Vector){-1,  3}));
    assert(vec_isclose(void_to_vec(list_get(tri, 1)), (Vector){2.2, 0.6}));
    assert(vec_isclose(void_to_vec(list_get(tri, 2)), (Vector){4,   3}));
    assert(isclose(polygon_area(tri), 6));
...
List *make_big_circ() {
    List *c = list_init(CIRC_NPOINTS, (FreeFunc) vec_free);
    for (size_t i = 0; i < CIRC_NPOINTS; i++) {
        double angle = 2 * M_PI * i / CIRC_NPOINTS;
        list_add(c, vec_init(cos(angle), sin(angle)));
    }
    return c;
}
...
List *make_weird() {
    List *w = list_init(5, (FreeFunc) vec_free);
    list_add(w, vec_init(0,  0));
    list_add(w, vec_init(4,  1));
    list_add(w, vec_init(-2, 1));
    list_add(w, vec_init(-5, 5));
    list_add(w, vec_init(-1, -8));
    return w;
}
...
void test_weird_translate() {
    ...
    assert(vec_isclose(void_to_vec(list_get(w, 0)), (Vector){-10, -20}));
    assert(vec_isclose(void_to_vec(list_get(w, 1)), (Vector){-6,  -19}));
    assert(vec_isclose(void_to_vec(list_get(w, 2)), (Vector){-12, -19}));
    assert(vec_isclose(void_to_vec(list_get(w, 3)), (Vector){-15, -15}));
    assert(vec_isclose(void_to_vec(list_get(w, 4)), (Vector){-11, -28}));
    assert(isclose(polygon_area(w), 23));
...
void test_weird_rotate() {
    ...
    assert(vec_isclose(void_to_vec(list_get(w, 0)), (Vector){2,  2}));
    assert(vec_isclose(void_to_vec(list_get(w, 1)), (Vector){1,  6}));
    assert(vec_isclose(void_to_vec(list_get(w, 2)), (Vector){1,  0}));
    assert(vec_isclose(void_to_vec(list_get(w, 3)), (Vector){-3, -3}));
    assert(vec_isclose(void_to_vec(list_get(w, 4)), (Vector){10, 1}));
    assert(isclose(polygon_area(w), 23));
